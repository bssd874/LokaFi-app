...
class TransactionController extends Controller
{
    ...
    public function store(StoreTransactionRequest $request): JsonResponse
    {
        ...
        $transaction = DB::transaction(function () use ($request, $data) {
            ...
            $transaction = $request->user()->transactions()->create([
                ...
                'normalized_merchant' => $this->normalizer->canonicalMerchant(
                    $data['merchant'] ?? null,
                ...
                'normalized_description' => $this->normalizer->descriptionSignature(
                    $data['description'] ?? $data['note'] ?? $data['merchant'] ?? '',
                    $data['merchant'] ?? null,
                ),
                'categorization_status' => $hasCategory ? 'categorized' : 'unclassified',
                'category_source' => $hasCategory ? 'user' : 'unclassified',
                'categorization_confidence' => $hasCategory ? 'high' : null,
                'categorization_confidence_score' => $hasCategory ? 100 : null,
                'categorization_explanation' => $hasCategory ? 'Kategori dipilih manual saat transaksi dibuat.' : null,
            ...
            return $transaction->load(['wallet', 'fromWallet', 'toWallet', 'category', 'suggestedCategory', 'categoryLabel']);
        });
    ...
    public function show(Transaction $transaction): JsonResponse
    {
        $this->ensureTransactionBelongsToUser($transaction);
            ...
            'data' => $transaction->load(['wallet', 'fromWallet', 'toWallet', 'category', 'suggestedCategory', 'categoryLabel']),
        ]);
    }
    ...
    public function update(UpdateTransactionRequest $request, Transaction $transaction): JsonResponse
    {
        ...
        $updatedTransaction = DB::transaction(function () use ($request, $transaction, $data) {
            ...
            $transaction->update([
                ...
                'normalized_merchant' => $this->normalizer->canonicalMerchant(
                    $mergedData['merchant'] ?? null,
                    $mergedData['description'] ?? $mergedData['note'] ?? null,
                ),
            ...
            $freshTransaction->update([
                ...
                'categorization_confidence_score' => null,
                'categorization_explanation' => null,
                'categorized_at' => null,
            ]);

            return $freshTransaction->fresh()->load(['wallet', 'fromWallet', 'toWallet', 'category', 'suggestedCategory', 'categoryLabel']);
        });
    ...
    public function unclassified(Request $request): JsonResponse
    {
        ...
        $query = $request->user()
            ->transactions()
            ->with(['wallet', 'category', 'suggestedCategory', 'categoryLabel'])
    ...
    public function updateCategory(
        ...
        try {
            $updatedTransaction = $this->categorizationService->correctCategory($transaction, $category);
        } catch (\Throwable $exception) {
            Log::warning('Deterministic correction failed, falling back to manual category update', [
    ...
    public function bulkCategory(BulkCategorizeTransactionsRequest $request): JsonResponse
    {
        ...
        foreach ($transactions as $transaction) {
            try {
                $this->categorizationService->correctCategory($transaction, $category);
                $updated++;
            } catch (\Throwable) {
                $skipped++;
            }
