...
class HttpAiProviderClient implements AiProviderClientInterface
{
    ...
    private function financialInsightSystemPrompt(): string
    {
        return implode(' ', [
            ...
            'Every highlight object must contain exactly type, title, description, and evidence_keys.',
            'Never omit evidence_keys; use an empty JSON array only when no supplied metric supports the highlight.',
            'Every recommended action object must contain exactly priority, title, description, and related_metric.',
            'Use only evidence_keys supplied by the user payload.',
            'Every recommended_actions item must include related_metric.',
            'Set related_metric to an exact key from allowed_evidence_keys, or set it to null when no appropriate key is available.',
            'Never invent metric keys or use aliases that are absent from allowed_evidence_keys.',
